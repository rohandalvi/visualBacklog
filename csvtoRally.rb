...
def create_stories(row)
	...
	if(row["Name"] == nil)
		...
		if !story_exists_in_rally(row["Name"])
			...
			match = row["Name"][/US(?<match>.*)\d/]
			puts "Match: #{match.to_s}"
			create_story["Description"] = row["Description"]
			@rally.update("hierarchicalrequirement","FormattedID|#{match}",create_story)
		end
	end
end

def story_exists_in_rally(story_id)
	if(!it_is_parent_ID(story_id))
		return false
	else
...
def build_query(type,fetch,string)
	...
	query.project ={"_ref" => "https://rally1.rallydev.com/slm/webservice/v2.0/project/13841632222.js"}
